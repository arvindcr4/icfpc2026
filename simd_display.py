...
def run(text, inputs, max_ticks=2_000_000, trace=0):
    ...
    disp = [r for r in rooms if any(g[r[1]][x]=='=' for x in range(r[0]+1,r[2]))]
    D = disp[0] if disp else None
    DW, DH = (D[2]-D[0]-1, D[3]-D[1]-1) if D else (0,0)
    cur = [0]; nxt = [[0]*DW for _ in range(DH)]; curbuf=[[0]*DW for _ in range(DH)]
    frames=[]
    dpipes={}
    if D:
        for p in pipes:
            if p.dst is D:
                ex,ey = p.cells[-1]
                fx,fy = ex+(p.cells[-1][0]-p.cells[-2][0]), ey+(p.cells[-1][1]-p.cells[-2][1])
                side = 'top' if fy==D[1] else 'bot' if fy==D[3] else 'left' if fx==D[0] else 'right'
                dpipes[side]=p
    ticks = 0
    while ticks < max_ticks and any(m.alive for m in men):
        ...
        if D:
            for side in ('top','left','bot'):
                p = dpipes.get(side)
                if p is None or p.slots[-1] is None: continue
                v = p.slots[-1]; p.slots[-1]=None
                if side=='top': cur[0]=v
                elif side=='left':
                    nxt[cur[0]//DW][cur[0]%DW]=v; cur[0]=(cur[0]+1)%(DW*DH)
                else:
                    curbuf=[row[:] for row in nxt]
                    frames.append([''.join('%x'%c for c in row) for row in curbuf])
                    if v==0:
                        nxt=[[0]*DW for _ in range(DH)]; cur[0]=0
        # 2. I/O
