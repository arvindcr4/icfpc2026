...
def test_man(man_code):
    ...
    for inp_str, expected in test_cases:
        ...
        try:
            out, ticks = run(man_code, inputs, max_ticks=100000, trace=0)
            if out == expected:
                print(f"PASS: {inp_str[:20]}... -> {out} ({ticks} ticks)")
            else:
                print(f"FAIL: {inp_str[:20]}...")
                print(f"  Got:      {out}")
                print(f"  Expected: {expected}")
        except Exception as e:
            print(f"ERROR on {inp_str[:20]}: {e}")
