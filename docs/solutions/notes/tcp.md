...
[Solve in editor →](https://icfpcontest2026.com/problems/tcp/editor)
...
A test case carries one stream of `n` packets, numbered `seq = 0 .. n-1`, each with a value `val`. [Round](https://icfpcontest2026.com/grading#round-based) 1 delivers `n`, then the first packet; every later round delivers one more packet — `seq val` — and the packets arrive in some scrambled order.
