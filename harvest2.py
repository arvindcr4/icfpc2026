...
RUNS = {
    ...
    "subset-sum":               "6a65e751",   # redesigned prompt (include-first DFS)
    "pathfinder":               "6a65d6e5",
    "matmul":                   "6a65e6db",   # redesigned prompt (i,t,j sequential FIFOs)
}
...
def cdp(conv, js):
    ...
    # cdp.js JSON-encodes its eval result, so a JSON string arrives twice-wrapped:
    # the first loads() yields a str, the second yields the real object.
    for _ in range(3):
